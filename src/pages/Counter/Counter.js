        ...
        {counterState.saved.map((item, index) => {
          return (
            <div key={index} className="saved_values">
              ...
              <p>
                Brojac:<span>{item.counted.counter}</span>
              </p>
              <p>
                Korisnik koji je sacuvao Podatak:<span>{item.fullName}</span>
              </p>
              <p>
                Datum Sacuvanog podatka:
                <span>
                  {item.datum.toLocaleString("en-US", { timeZone: "UTC" })}
                </span>
              </p>
            </div>
          );
